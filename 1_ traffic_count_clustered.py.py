...
# Step 4: Add markers to the map with traffic count as popup, coordinates, and color coding
for idx, row in df.iterrows():
    ...
    folium.Marker(
        ...
        popup=f"<b>Traffic Count:</b> {traffic_count}<br>{row['Latitude']}, {row['Longitude']}",  # Separate with <br>
        icon=folium.Icon(color=color, icon='car', prefix='fa')  # Use Font Awesome 'car' icon
...
m.save('traffic_count_map_with_car_icons_and_legend.html')

# Print a message to indicate that the map has been saved
print("Map has been saved as 'traffic_count_map_with_car_icons_and_legend.html'.")
