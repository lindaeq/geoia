...
map_pedestrian = folium.Map(location=[df_pedestrian['Latitude'].mean(), df_pedestrian['Longitude'].mean()], zoom_start=15)

# Step 3: Create a MarkerCluster to group nearby markers
marker_cluster_pedestrian = MarkerCluster().add_to(map_pedestrian)

# Step 4: Add markers to the map with pedestrian count as popup and person icon
for idx, row in df_pedestrian.iterrows():
    ...
    ).add_to(marker_cluster_pedestrian)
...
map_pedestrian.save('pedestrian_count_map_with_person_icon_and_legend.html')

# Print a message to indicate that the map has been saved
print("Map has been saved as 'pedestrian_count_map_with_person_icon_and_legend.html'.")
